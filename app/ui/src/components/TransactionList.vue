...
<template>
  <div class="w-full flex flex-col items-center space-y-20">
    <SearchInput v-model:address="address" :loading="loading" @search="fetchTransactions(true)" />

    <div v-if="error" class="text-red-600">{{ error }}</div>

    <TransactionAnalysis :analyzing="analyzing" :analysis="analysis" />
    <GraphSection v-if="transactions.length > 0" :transactions="transactions" :address="address" />
